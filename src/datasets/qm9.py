    ...
    **STATS:**
    ...
    def process(self) -> None:
        ...
        for i, mol in enumerate(tqdm(suppl)):
            ...
            # compute position
            pos = [c.GetPositions()[:, :2] for c in mol.GetConformers()][0]
...
def create_dataset(config: DataConfig, dev: bool = False, force_reload=False):
    ...
    # Max radius for qm9 in 2D is tensor(7.9607, dtype=torch.float64)
    ...
    for idx, data in enumerate(dataset):
        ...
        x = torch.tensor(data.pos, dtype=torch.float32).cuda()
        x = x - x.mean(axis=0)
        x = x / x.norm(dim=-1).max()

        # NOTE: Gets only the middle 3 channels.
        ects = ect_tr(
            x,
